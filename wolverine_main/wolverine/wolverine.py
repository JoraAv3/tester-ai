...
def run_script(script_name: str, script_args: List) -> str:
    print("script", script_name, script_args)
    """
    If script_name.endswith(".py") then run with python
    else run with node
    """
    script_args = [str(arg) for arg in script_args]
    subprocess_args = [sys.executable, script_name, *script_args]
    # subprocess_args = (
...
def send_message_to_gpt(
    message: str, args: List, error_message: str, model: str = DEFAULT_MODEL
) -> Dict:
...
def apply_message_changes(message: str, changes: List, confirm: bool = False):
    """
    Pass changes as loaded json (list of dicts)
    """
    original_file_lines = message.split("\n")
